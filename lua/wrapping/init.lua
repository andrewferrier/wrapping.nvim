...
---@param mode string
---@return nil
local function emit_wrapping_set_event(mode)
...
local function likely_textwidth_set_deliberately()
    ...
    local info_global = get_info("textwidth", { scope = "global" })
    local info_local = get_info("textwidth", {})

    -- textwidth has probably been set by a modeline, autocmd or
    -- ftplugin/x.{lua.vim} deliberately
    local is_same = (
        textwidth_global == textwidth_buffer
        and info_global.last_set_sid == info_local.last_set_sid
        and info_global.last_set_linenr == info_local.last_set_linenr
    )
...
M.setup = function(o)
    ...
    if opts.create_keymaps then
        vim.keymap.set(
            ...
            { desc = "Soft wrap mode", unique = true }
        )
        vim.keymap.set(
            "n",
            "]ow",
            "<Plug>(wrapping-hard-wrap-mode)",
            { desc = "Hard wrap mode", unique = true }
        )
        vim.keymap.set(
            "n",
            "yow",
